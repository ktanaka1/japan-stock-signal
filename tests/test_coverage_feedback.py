...
def test_classify_priority():
    items = _items("1111", "2222", "3333", "4444")
    ...
    assert st["2222"] == matcher.STATUS_SIGNALED
...
def test_load_window_reads_signals_and_analyses():
    ...
    delivered, filtered, analyzed = matcher.load_window(since, until)
...
def test_agent_run_records_coverage(monkeypatch):
    ...
    # 窓内に 6533 の配信済みシグナルを置く（_window が拾える時刻に）
    monkeypatch.setattr(agent.matcher, "load_window", lambda s, u: ({"6533"}, set(), set()))

    agent.run()

    row = execute("SELECT * FROM coverage_runs ORDER BY id DESC LIMIT 1").rows[0]
    assert row["ranking_type"] == "up"
    assert row["universe"] == 2          # ETF除外で母集団2
    assert row["captured"] == 1          # 6533
    assert row["not_collected"] == 1     # 4444
    assert row["capture_rate"] == 0.5
